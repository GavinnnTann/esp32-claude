...
MOODS: dict[str, dict] = {
    ...
    # Opus below high effort: the same guitar, played for the pleasure of it.
    # Deliberately the OPPOSITE face to rocking - raised brows and a smile
    # against rocking's angry brows and narrow slits - because the two sat side
    # by side as one model's pair and the guitar alone did not tell them apart.
    # No stage either; the purple backdrop now means "performing hard" and
    # nothing else, so this one plays on black.
    "rocking_calm": dict(eye="open",   brow="raised", pupil_dx=0, mouth="smile", wave=0, bob=3,
                         speed=1.3, guitar=True),
